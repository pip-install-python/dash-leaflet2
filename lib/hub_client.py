...
def app_id() -> str:
    ...
    ``leaflet`` is this app's 2plot network-directory key — the same value
    render.yaml sets for ``SATELLITE_APP_KEY`` and ``AD_APP_ID``, and the one
    the hub labels its series and its page-tier ceilings by. The literal is
    the fallback for a local run with neither variable set; getting it wrong
    would make the hub answer for a different app's pages.
    """
    return os.getenv("SATELLITE_APP_KEY") or os.getenv("AD_APP_ID") or "leaflet"
