...
Env:
    ...
    SATELLITE_APP_KEY           network directory key for this app (default
                                "leaflet", this app's own directory key)
...
def app_key() -> str:
    ...
    now agree ("leaflet" in both). This host historically ran
    ``AD_APP_ID=dash-leaflet2`` (the ad network's long identifier) against
    directory key "leaflet", and setting one for ads must never silently
    re-key its analytics series off the directory. The convergence today is a
    convenience, not a contract to lean on.

    Default is "leaflet" — the hub's directory key for this app, NOT the
    package name ``dash-leaflet2``, which the hub only still accepts as a
    legacy id. A rename here silently orphans the traffic series.
    """
    return os.getenv("SATELLITE_APP_KEY") or "leaflet"
